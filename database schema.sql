...
CREATE TABLE public.applications (
  ...
  status text DEFAULT 'pending'::text CHECK (status = ANY (ARRAY['pending'::text, 'in-progress'::text, 'rejected'::text, 'maybe'::text, 'archived'::text])),
...
CREATE TABLE public.jobs (
  ...
  created_at timestamp with time zone DEFAULT now(),
  updated_at timestamp with time zone DEFAULT now(),
  what_you_will_do ARRAY NOT NULL DEFAULT '{}'::text[],
...
CREATE TABLE public.users (
  ...
  user_role text NOT NULL DEFAULT 'talent'::text CHECK (user_role = ANY (ARRAY['superadmin'::text, 'admin'::text, 'owner'::text, 'talent'::text, 'recruiter'::text, 'interviewer'::text])),
  is_active boolean DEFAULT true,
