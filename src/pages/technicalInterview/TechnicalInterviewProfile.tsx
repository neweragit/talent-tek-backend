...
type TechnicalInterviewerProfileState = {
  fullName: string;
  email: string;
  role: string;
};

const DEFAULT_TECHNICAL_INTERVIEWER_PROFILE: TechnicalInterviewerProfileState = {
  fullName: "",
  email: "",
  role: "",
};
...
const TechnicalInterviewProfile = () => {
  ...
  const [profile, setProfile] = useState<TechnicalInterviewerProfileState>(DEFAULT_TECHNICAL_INTERVIEWER_PROFILE);
  ...
  const profileInitials = `${profile.fullName?.[0] ?? "T"}${profile.fullName?.[profile.fullName.lastIndexOf(" ") + 1] ?? "I"}`.toUpperCase();
    ...
    if (isEditing) {
      ...
      if (error) {
        ...
        toast({
          ...
          description: "Your technical interviewer profile has been saved.",
        });
      }
    }
